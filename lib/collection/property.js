 ...
 */
/**
 * The Property class forms the base of all postman collection SDK elements.
 * This is to be used only for SDK development or to extend the SDK with
 * additional functionalities. All SDK classes (constructors) that are supposed
 * to be identifiable (i.e. ones that can have a `name` and `id`) are derived
 * from this class.
 *
...
class Property extends PropertyBase {
    ...
    toObjectResolved (scope, overrides, options) {
        let ignoreOwnVariables = options && options.ignoreOwnVariables,
            variableSourceObj,
            variables,
            reference;
    ...
    static replaceSubstitutions (str, variables) {
     ...
     * @returns {Object}
     */
    static replaceSubstitutionsIn (obj, variables, mutate) {
        // if there is nothing to replace, we move on
        ...
        let customizer = function (objectValue, sourceValue) {
 ...
 * @type {String}
 */
Property._postman_propertyName = 'Property';
