 ...
 * }
 */
class ItemGroup extends Property {
    /**
    ...
    forEachItem (callback) {
        this.items.each(function (item) {
            return ItemGroup.isItemGroup(item) ? item.forEachItem(callback) : callback(item);
        }, this);
    }
     ...
     * @param {Function} callback
     */
    forEachItemGroup (callback) {
        this.items.each(function (item) {
            if (ItemGroup.isItemGroup(item)) {
                item.forEachItemGroup(callback);
                callback(item); // eslint-disable-line callback-return
            }
        }, this);
    }
