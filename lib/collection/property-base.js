 ...
 */
/**
 * Base of all properties in Postman Collection.
 * It defines the root for all standalone properties for postman collection.
 *
 * @private
 */
class PropertyBase {
    ...
    constructor (definition) {
        ...
        // Also if the definition is basic string we do not need to do anything with it.
        if (!definition || typeof definition === 'string') { return; }

        // call the meta extraction functions to create the object where all
        // keys that are prefixed with underscore can be stored.
        // more details on that can be retrieved from the propertyExtractMeta function itself.
    ...
    findInParents (property, customizer) {
        let owner = this.findParentContaining(property, customizer);
     ...
     * @returns {boolean}
     */
    static propertyIsMeta (value, key) {
     ...
     * @returns {String}
     */
    static propertyUnprefixMeta (value, key) {
 ...
 * @type {String}
 */
PropertyBase._postman_propertyName = 'PropertyBase';
